...
import { Eye, Building2, User, FileText, Clock, MessageSquare, Flag, AlertCircle, Search } from "lucide-react";
import { Input } from "@/components/ui/input";

export default function IntegratedRecords() {
  const integrated = stagingRecords.filter((r) => r.status === "approved");
  const [selected, setSelected] = useState<StagingRecord | null>(null);
  const [search, setSearch] = useState("");

  const filtered = integrated.filter(
    (r) =>
      r.patientName.toLowerCase().includes(search.toLowerCase()) ||
      r.nin.includes(search) ||
      r.sourceHospital.toLowerCase().includes(search.toLowerCase())
  );
